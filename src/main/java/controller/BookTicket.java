...
public class BookTicket extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (user == null) {
			resp.getWriter().print("<h1 style='color:red'>Login First</h1>");
			...
			if (numberOfSeats <= 0) {
				...
				if (from.equals(to)) {
					...
					if (fromstation > tostation) {
						...
						double price = Double.parseDouble(train.getPrice()[tostation]);
						...
						if (Period.between(dob.toLocalDate(), doj.toLocalDate()).getDays() < 0 || flag) {
							...
							req.getRequestDispatcher("BookTicket.jsp").include(req, resp);
						} else {
